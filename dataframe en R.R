...
names(peliculas_df) <- c('NOMBRE', 'PUNTUACIÓN', 'POSTERIOR_2005')
...
order(peliculas_df$PUNTUACIÓN)

# funcion order (menor a mayor)
order_menor_mayor <- order(peliculas_df$PUNTUACIÓN, decreasing=F)
...
order_mayor_menor <- order(peliculas_df$PUNTUACIÓN, decreasing=T)
